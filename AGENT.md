...
10. **완료 전 게이트.** `npm run build`, `npm test`, `npm run test:e2e`, `npm run lint` 통과 +
...
| 항목 | 값 |
|---|---|
| 런타임 | Node.js **22** (Dockerfile `node:22-alpine`, CI `node-version: 22`) |
...
├── .github/workflows/deploy.yml  [Platform] CI/CD (test → docker/GHCR)
...
1. **백엔드 타입 엄격도 불일치.** `tsconfig.json`이 full `strict`가 아니며
   (`noImplicitAny: false` 등), ESLint `@typescript-eslint/no-explicit-any: 'off'`.
   프론트(`frontend/tsconfig.json`)는 `strict: true`. **목표:** 백엔드도 full `strict` +
   `no-explicit-any` 복원.
...
| `npm run lint` | ESLint(`{src,apps,libs,test}/**/*.ts`, `--fix`) — **백엔드 전용** |
| `npm run format` | Prettier(`src/**`, `test/**`) — **백엔드 전용** |
